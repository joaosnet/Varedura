...
def quick_cleanup(console: Optional[Console] = None):
    ...
    overall_task = overall_progress.add_task(
        "[cyan]Executando limpeza rápida...", total=100
    )
    with Live(progress_group, refresh_per_second=10, console=console):
        try:
            ...
            overall_progress.update(
                overall_task, description="[cyan]Limpando sistema Docker..."
            )
            run_cmd(
                console,
                "docker system prune -af --volumes",
                "Limpando sistema Docker (agressivo)",
            )
            ...
            overall_progress.update(
                overall_task, description="[cyan]Compactando arquivo VHDX..."
            )
            ...
            if os.path.exists(vhdx_path):
                ...
                if run_cmd(
                    ...
                    if os.path.exists(vhdx_path):
                        ...
                        console.print(
                            f"[bold]Espaço economizado:[/bold] {space_saved:.2f} GB"
                        )
            else:
                console.print(
                    f"[yellow]Arquivo VHDX não encontrado: {vhdx_path}[/yellow]"
                )
            current_task_progress.remove_task(current_task)
            overall_progress.update(overall_task, advance=30)
            overall_progress.update(

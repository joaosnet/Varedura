...
class ModelsGenerator:
    @staticmethod
    def extract_initial_models(
        raw_data: str, output_stream: Optional[TextIO] = None
    ) -> List[Dict[str, Any]]:
        models = []
        if "initialModels" in raw_data:
            try:
                ...
                m = re.search(
                    r"initialModels\s*:\s*(\[[\s\S]*?\])", raw_data, re.DOTALL
                )
                if m:
                    json_str = m.group(1)
                else:
                ...
                if json_str:
                    try:
                        ...
                        try:
                            json_str_decoded = bytes(json_str, "utf-8").decode(
                                "unicode_escape"
                            )
    ...
    def format_models_python(
        ...
        for i, model in enumerate(models):
            ...
            if "organization" in normalized:
                fields.append(
                    f"'organization': {json.dumps(normalized['organization'])}"
                )
    ...
    def generate_full_code(
        ...
        models_code = ModelsGenerator.format_models_python(
        ...
        text_models_code = (
            f"text_models = {json.dumps(text_models, ensure_ascii=False)}"
        )
        image_models_code = (
            f"image_models = {json.dumps(image_models, ensure_ascii=False)}"
        )
        vision_models_code = (
            f"vision_models = {json.dumps(vision_models, ensure_ascii=False)}"
        )
        code = f"""{models_code}
    ...
    if is_tty:
        try:
            ...
            with Progress() as progress:
                ...
                code = ModelsGenerator.generate_full_code(
                    models, output_stream, progress_callback=rich_handler
                )
        except ImportError:
            code = ModelsGenerator.generate_full_code(models, output_stream)
    else:
        code = ModelsGenerator.generate_full_code(
            models, output_stream, progress_callback=progress_handler
        )

    output_stream.write(code)
    output_stream.write("\n")
    output_stream.flush()
    output_file = input_file.replace(".json", "_models.py").replace(
        ".txt", "_models.py"
    )
